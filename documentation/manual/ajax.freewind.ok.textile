...
Play在内部使用了 "jQuery":http://jquery.com 这个JavaScript库，可以让我们非常方便的进行Ajax操作。为了能够在JavaScript中方便地生成某个action对应的Url，Play还提供了一个 @jsAction@ 标签，简化操作。

h2. <a name="jsaction">配合jQuery使用jsAction标签</a>

在play的页面中，我们想取得某action对应的url，通常不会手动拼出来，而是像Java中那样写成一个函数调用，由Play解析并生成。这么做的好处是，可以让Play检查我们的调用有没有错误，不会产生错误的URL。
